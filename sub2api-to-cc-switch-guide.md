# SubForAI 导入到 CC Switch 教程

这篇教程面向 0 基础用户，按顺序照着点就可以。  
本文里的 `CC Switch`、`CC-Switch`、`CCS` 指的是同一个东西；在当前 SubForAI 界面里，按钮名称是 `导入到 CCS`。
...
登录 SubForAI 后，在左侧菜单找到 `API Keys`。  
...
这一步你不需要手动复制一大堆配置，SubForAI 会把必要信息直接交给 CC Switch。

### 哪些信息会被自动带过去

SubForAI 会根据你的分组平台，自动决定导入到哪个客户端类型：
...
SubForAI 当前的实现会调用类似 `ccswitch://...` 的唤起链接；如果系统不认识这个协议，就不会正常打开 CC Switch。

### 2. 页面里根本没有“导入到 CCS”按钮

这通常不是你点错了，而是站点管理员把这个按钮隐藏了。  
SubForAI 后台有一个开关叫 `隐藏 CCS 导入按钮`，打开后，用户侧就看不到这个按钮。
...
> 登录 SubForAI -> 打开 `API Keys` -> 没有密钥就先创建 -> 点击 `导入到 CCS` -> 如果弹出客户端选择，就按你实际使用的客户端来选。
